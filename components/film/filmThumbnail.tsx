...
export default function FilmThumbnail(props: Props) {
  const { film } = { ...props };
  return (
    <div className="mx-auto hover:z-10 transition ease-in-out hover:scale-110 hover:cursor-pointer">
      <ImageWithBlur
        src={film.thumbnailPath}
        width={100}
        height={100}
        alt={`${film.title} thumbnail`}
        className="hover:shadow-md hover:shadow-sky-400  h-44 w-full rounded-md drop-shadow-lg object-cover"
      />
      <div className="flex">
