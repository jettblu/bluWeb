...
const RecentDocCard = ({ doc }: Props) => {
  const urlBase = "/blog/[slug]";
  const urlAs = `/blog/${doc.slug}`;
  const formattedDate = new Date(doc.lastUpdate).toDateString();
  return (
    <div className="mx-auto w-full">
      <div className="rounded-lg bg-white dark:bg-gray-900 text-black dark:text-white outline outline-1 outline-gray-500 hover:outline-green-400 min-h-[300px] max-h-[300px] md:min-h-[500px] md:max-h-[500px]">
