            ...
            <div className="max-w-2xl mx-auto">
                ...
                <div>
                    ...
                    <DocContent content={doc.content}/>
                    {
                      readNext.length!=0 &&
                      <div className="my-8">
                        <h1 className="font-bold text-3xl text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-emerald-600 text-left">Keep Reading</h1>
                        <hr className="mt-2 mb-4"/>
                        <div className="flex flex-col space-y-4">
                        {
                          readNext.map((doc:DocType, index:number)=>
                          <DocKeepReadingPreview baseUrl="/blog/" key={`keep reading preview ${index}`} title={doc.title} image={doc.image||undefined} emoji={doc.emoji||undefined} oneLiner={doc.oneLiner} slug={doc.slug}/>
                          )
                        }
                        </div>
                    </div>
